...
func (ke1 *KE1) UpdateData() (ret error) {
	/* Get Voltage */
	var stopBits int = 1
	results, err := ke1.ms.ReadHoldingRegisters(ke1.id, 0, 2*numberOfVoltage, stopBits)
	...
	/* Get Current */
	results, err = ke1.ms.ReadHoldingRegisters(ke1.id+1, 0x0C, 2*numberOfCurrent, stopBits)
	...
	/* Get Frequency */
	results, err = ke1.ms.ReadHoldingRegisters(ke1.id, 0x34, 2, stopBits)
	...
	/* Get PowerFactor */
	results, err = ke1.ms.ReadHoldingRegisters(ke1.id+1, 0x24, 2*numberOfPowerFactor, stopBits)
	...
	/* Get EffectivePower */
	results, err = ke1.ms.ReadHoldingRegisters(ke1.id+1, 0x38, 2*numberOfEffectivePower, stopBits)
	...
	results, err = ke1.ms.ReadHoldingRegisters(ke1.id+1, 0x100, 2*numberOfIntegratedElectricEnergy, stopBits)
